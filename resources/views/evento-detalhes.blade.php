...
<div x-data="{ open:false, ingressoNome:'', ingressoPreco:0, ingressoId:'', quantidade:1 }"
...
Restam {{ $tipo->quantidade_disponivel }} bilhetes
</p>
...
style="width: {{ ($tipo->quantidade_disponivel / ($tipo->quantidade_disponivel + 10)) * 100 }}%">
</div>
...
{{ number_format($tipo->preco,0,',','.') }} Kz
</p>

<button 
    @click="$dispatch('abrir-modal', { nome: '{{ $tipo->nome }}', preco: {{ $tipo->preco }}, id: '{{ $tipo->id }}' })"
...
<div
    ...
    class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-lg"
>
    <div
